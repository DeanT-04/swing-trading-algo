...
class RiskManager:
    ...
    def calculate_position_size(self, symbol: str, price: float, balance: float) -> Tuple[float, float]:
        ...
        """
        # Calculate trade budget (10% of current balance)
        trade_budget = balance * self.max_position_pct

        # Calculate position size
        if price > 0:
            # Calculate how many shares/contracts we can buy with the trade budget
            position_size_raw = trade_budget / price

            # For expensive stocks (>£50), allow partial shares
            if price > 50.0:
                # Round to 2 decimal places for partial shares
                position_size = round(position_size_raw, 2)
            ...
            # Ensure we don't exceed trade budget
            if cost > trade_budget * 1.05:  # Allow 5% buffer for rounding
                # Recalculate position size to stay within budget
                position_size = trade_budget / price
                if price > 50.0:
            ...
            # Log the position size and cost
            allocation_pct = (cost / balance * 100) if balance > 0 else 0.0
            logger.info(f"Position size for {symbol} at £{price:.2f}: {position_size} shares, cost: £{cost:.2f} ({allocation_pct:.1f}% of balance)")
